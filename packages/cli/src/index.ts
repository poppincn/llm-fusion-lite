...
loadEnv(); // pick up ~/.llm-fusion-lite/.env or ./.env (never overrides real env vars)
...
program
    .name("fusion-lite")
    .description("LLM Fusion Lite — multi-model synthesis with adaptive, learned model strengths.")
    .version("0.1.0");
            ...
            switch (e.type) {
                case "preflight":
                    for (const m of e.missing) {
                        log(
                            chalk.yellow(`  ⚠ skipped ${m.modelId}${m.reason ? `: ${m.reason}` : " (no credentials)"}`)
                        );
                    }
                    break;
                case "category":
                ...
                case "panel_done": {
                    ...
                    if (r.error) log(chalk.red(`  ✗ ${r.label}: ${r.error}`));
                        ...
                        log(
                            chalk.green(`  ✓ ${r.label}`)
                                + chalk.dim(
                                    ` (${r.latencyMs}ms${r.citations?.length ? `, ${r.citations.length} sources` : ""})`
                                )
                        );
                    break;
                }
                ...
                log(
                    chalk.dim(
                        `\n— run ${runId.slice(0, 8)} · ${meter}`
                            + ` · feedback: fusion-lite feedback ${runId.slice(0, 8)} up|down`
                    )
                );
            }
        } catch (e) {
...
program
    .command("serve")
    .description("start the LLM Fusion Lite server (OpenAI-compatible endpoint + web UI)")
    .option("--port <n>", "port", v => parseInt(v, 10))
    .action(async opts => {
        const { startServer } = await import("@llm-fusion-lite/server");
        const { port } = startServer({ port: opts.port });
        log(chalk.green(`\n  LLM Fusion Lite → http://localhost:${port}`));
...
program
    ...
    .action((category?: string) => {
        const store = new FusionStore();
        const strengths = store.getStrengths(category).sort((a, b) => b.score - a.score);
        store.close();
        if (!strengths.length) {
...
program
    ...
    .action((runId: string, rating: string, modelId?: string) => {
        const r =
            /^up|good|\+1?$/i.test(rating) ? 1
            : /^down|bad|-1?$/i.test(rating) ? -1
            : 0;
        if (r === 0) {
 ...
 * Mirrors CLI_SPECS in @llm-fusion-lite/core; kept here for the wizard's install +
 * version flows and the doctor readout (these need bin/pkg + login hints).
 */
const PROVIDER_CLI: Partial<Record<ProviderName, { bin: string; pkg: string; loginHint: string }>> = {
...
const ENGINE_PKG = "llm-fusion-lite";
...
program
    ...
    .action(async opts => {
        ...
        for (const { name, label } of PROV_LABELS) {
            ...
            if (mode === "subscription") {
                ...
                if (!present) {
                    ...
                    const auth =
                        signedIn === true ? chalk.green("signed in")
                        : signedIn === false ? chalk.red(`NOT signed in — ${cli.loginHint}`)
        ...
        const customProviders = (config.providers ?? []).filter(p => p.adapter === "openai-compatible");
        if (customProviders.length) {
            log(chalk.bold("\nCustom providers (OpenAI-compatible)"));
            for (const p of customProviders) {
                const keyEnv = p.apiKeyEnv ?? "BASETEN_API_KEY";
                const ready = !!p.baseURL && !!process.env[keyEnv];
                const detail =
                    p.baseURL ?
                        `${p.name} · ${p.baseURL} · key ${ready ? "set" : `missing (${keyEnv})`}`
                    :   `${p.name} · no baseURL`;
                log(`  ${opt(ready)} ${p.id.padEnd(22)} ${chalk.dim(detail)}`);
            }
        }
        ...
        if (available.length >= 2) {
            ...
            log(
                `  ${chalk.red("✗")} no providers. Set ANTHROPIC_API_KEY (and optionally OPENAI/GOOGLE), or install codex/gemini CLIs.`
            );
        }
            ...
            for (const id of available) {
                ...
                if (authModeFor(spec.provider, config) === "subscription") {
                    ...
                    if (probe.authenticated) {
                        log(
                            `  ${ok(true)} ${id.padEnd(20)} ${chalk.dim(`authenticated${probe.reason ? ` (${probe.reason})` : ""}`)}`
                        );
                    } else {
                ...
                try {
                    ...
                    log(
                        `  ${chalk.red("✗")} ${id.padEnd(20)} ${chalk.red((e instanceof Error ? e.message : String(e)).slice(0, 80))}`
                    );
                }
            }
        }
...
function readinessLine(config: FusionConfig): string {
    ...
    return chalk.red("✗ no providers — set a key with `fusion-lite setup` or export ANTHROPIC_API_KEY");
}
...
async function runSetupWizard(install: boolean): Promise<void> {
    p.intro(chalk.bold("LLM Fusion Lite setup"));

    // 1) Per-provider auth: API key, subscription (CLI login), or skip.
    for (const prov of SETUP_PROVIDERS) {
        const current = process.env[prov.env] || (prov.name === "google" ? process.env.GEMINI_API_KEY : "") || "";
        const cli = PROVIDER_CLI[prov.name];
        ...
        if (mode === "api") {
            const hint =
                current ? `set (${maskKey(current)}) — Enter to keep`
                : prov.required ? "required — paste key, or Enter to set later"
    ...
    if (engineLatest && engineLatest !== program.version()) {
        p.log.info(
            `fusion-lite v${program.version()} installed, v${engineLatest} available — update: ${chalk.cyan(
                `npm i -g ${ENGINE_PKG}`
            )}`
        );
    }

    p.outro(`Done. Try ${chalk.cyan('fusion-lite "your question"')}  ·  health: ${chalk.cyan("fusion-lite doctor")}`);
}
    ...
    .action(async opts => {
        ...
        if (skillOnly || !process.stdin.isTTY) {
            if (!skillOnly)
                log(
                    chalk.yellow(
                        "Non-interactive shell — installing the skill only. Run `fusion-lite setup` in a terminal to enter keys."
                    )
                );
            try {
                ...
                log(
                    chalk.dim(
                        "\nThe skill calls `fusion-lite-run` (on PATH from this package). Run `fusion-lite doctor` to verify keys.\n"
                    )
                );
            } catch (e) {
