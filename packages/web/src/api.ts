...
export async function streamFuse(
    ...
    if (!res.ok || !res.body) {
        ...
        throw new Error(`Fusion request failed (${res.status})${detail ? `: ${detail}` : ""}`);
    }
 ...
 */
function handleSseMessage(raw: string, onEvent: (event: FusionEvent) => void): boolean {
...
export async function getStrengths(category?: string): Promise<ModelStrength[]> {
    const qs = category && category.length > 0 ? `?category=${encodeURIComponent(category)}` : "";
...
export async function updateConfig(update: ConfigUpdate): Promise<FusionConfig> {
    const res = await fetch("/api/config", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
        ...
        throw new Error(`Config update failed (${res.status})${detail ? `: ${detail}` : ""}`);
    }
