...
export function SettingsBar({ config, settings, onChange, disabled }: Props) {
  ...
  const togglePanelMember = (id: string) => {
    const next = settings.panel.includes(id)
      ? settings.panel.filter((p) => p !== id)
      : [...settings.panel, id];
      ...
      <div className="settings-row">
        ...
        <label className="settings-inline">
          <input
            ...
            disabled={disabled}
            onChange={(e) =>
              onChange({ ...settings, webSearch: e.target.checked })
            }
          />
          Web search
        </label>
        ...
        <label className="settings-inline">
          ...
          <input
            ...
            onChange={(e) =>
              onChange({
                ...settings,
                panelSize: clampInt(e.target.value, 1, 12),
              })
            }
          />
        </label>
        ...
        <div className="settings-panel">
          <div className="settings-panel-title">
            ...
            <span className="muted">
              (leave all unchecked for adaptive selection)
            </span>
          </div>
          {availableModels.length === 0 ? (
            <div className="muted">No models available.</div>
          ) : (
