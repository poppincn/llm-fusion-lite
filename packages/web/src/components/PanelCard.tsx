        ...
        >
            ...
            >
                ...
                <span className="panel-card-meta">
                    {hasError ?
                        <span className="badge badge-error">{t("panel.error")}</span>
                    : panel.done ?
                        <>
                            {resp && <span className="badge">{formatLatency(resp.latencyMs)}</span>}
                            {sourceCount > 0 && (
                                <span className="badge">
                                    {t(sourceCount === 1 ? "panel.sourceOne" : "panel.sources", { n: sourceCount })}
                                </span>
                            )}
                        </>
                    :   <span className="badge badge-thinking">{t("panel.thinking")}</span>}
            ...
            {!collapsed && (
                <div className="panel-card-body">
                    ...
                    :   <div className="panel-text">
                            {panel.text.length > 0 ? panel.text : <span className="muted">{t("panel.waiting")}</span>}
                        </div>
                    }
                    ...
                    {resp && (
                        <div className="panel-card-foot muted">
                            {resp.usage?.outputTokens != null ?
                                t("panel.tokensOut", { provider: resp.provider, n: resp.usage.outputTokens })
                            :   resp.provider}
                        </div>
                    )}
                </div>
