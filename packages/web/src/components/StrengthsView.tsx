  ...
  const load = useCallback(async () => {
    ...
    try {
      const [s, r] = await Promise.all([
        getStrengths(category || undefined),
        getRuns(25),
      ]);
    ...
    <div className="strengths">
      ...
      <div className="strengths-grid">
        <section className="card">
          <h3 className="card-title">Scores by model{category ? ` · ${category}` : ""}</h3>
          {strengths.length === 0 ? (
            <p className="muted">
              {loading ? "Loading…" : "No strength data yet. Run some fusions."}
            </p>
          ) : (
            <table className="strengths-table">
              <thead>
                <tr>
                  ...
                  <th className="num">Avg contrib</th>
        ...
        <section className="card">
          ...
          {runs.length === 0 ? (
            <p className="muted">{loading ? "Loading…" : "No runs yet."}</p>
          ) : (
