...
const KEY_VARS = ["ANTHROPIC_API_KEY", "OPENAI_API_KEY", "GOOGLE_API_KEY", "GEMINI_API_KEY", "BASETEN_API_KEY"];
function clearKeys() {
    for (const k of KEY_VARS) delete process.env[k];
}

test("empty prompt is rejected before any model call", async () => {
    await assert.rejects(() => fuse({ prompt: "   \n\t " }, { config: DEFAULT_CONFIG }), /Empty prompt/);
});
