...
/** The API-key env var a model reads (per-model for openai-compatible endpoints). */
function apiEnvFor(spec: ModelSpec): string {
  if (spec.provider === "openai-compatible") return spec.apiKeyEnv ?? PROVIDER_META["openai-compatible"].env;
...
function apiKeyPresent(spec: ModelSpec): boolean {
  ...
  if (spec.provider === "google" && process.env.GEMINI_API_KEY) return true;
  return false;
}
...
async function checkModel(
  ...
  if (!spec) {
    return { modelId: id, provider: "unknown", mode: "api", ready: false, reason: `unknown model id "${id}" — not in the registry` };
  }
  ...
  if (mode === "subscription") {
    if (!cliAvailable(spec.provider)) {
      ...
      return { modelId: id, provider: spec.provider, mode, ready: false, reason: `${meta.cli ?? "CLI"} not on PATH${pkg}` };
    }
    ...
    return { modelId: id, provider: spec.provider, mode, ready: false, reason: `${probe.reason ?? "unauthenticated"}${login}` };
  }
  ...
  return { modelId: id, provider: spec.provider, mode, ready: false, reason: `no ${env} — export ${env}=… or run \`fuse setup\`` };
}
...
export function formatMissingCredentials(missing: CredentialStatus[]): string {
  return missing
    .map((s) => `  • ${s.modelId} (${s.provider}/${s.mode}): ${s.reason ?? "not configured"}`)
    .join("\n");
}
