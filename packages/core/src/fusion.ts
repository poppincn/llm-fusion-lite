...
import { formatMissingCredentials, modelCredentialReady, preflightCredentials } from "./credentials.js";
import { adjudicate } from "./adjudicate.js";
import { parseConfidence, refinePanel, pairwiseRank, selectConsistent, verifyAndRevise } from "./techniques.js";
...
export function availableAutoPanel(config: FusionConfig): string[] {
    return config.autoPanel.filter(id => {
        ...
        return spec && !spec.excludeFromAuto && modelCredentialReady(spec);
    });
}
    ...
    try {
        ...
        if (opts.agentic) {
            ...
            if (eligible.length === 0) {
                throw new Error(
                    "No models eligible for the agentic panel. Check the registry and that the sandbox is up."
                );
            }
        ...
        const panelSpecs = panelIds.map(id => getModel(config, id)).filter((s): s is ModelSpec => !!s);
        emit({ type: "panel_selected", panel: panelSpecs.map(s => ({ id: s.id, label: s.label })) });
            ...
            if (!jp.ok) {
                ...
                emit({
                    ...
                    missing: [
                        {
                            modelId: judge.id,
                            reason: `${jp.missing[0]?.reason ?? "no credentials"} — falling back to ${fallback.id} as judge`
                        }
                    ]
                });
                judge = fallback;
        ...
        if (tech.confidence) {
            extraSys.push({
                role: "system",
                content:
                    'End your answer with a final line exactly like "Confidence: 75%" — your calibrated confidence that it is correct.'
            });
        }
        ...
        if (tech.selfConsistency > 1) {
            ...
            const samples = await Promise.all(
                Array.from({ length: tech.selfConsistency }, () =>
                    runJudgeSynthesis(judge, messages, panel, analysis, {
            ...
            const idx = await selectConsistent(
                messages,
        ...
        const usage: FusionUsage = {
            inputTokens: panel.reduce((a, p) => a + (p.usage?.inputTokens ?? 0), 0) + judgeIn,
            outputTokens: panel.reduce((a, p) => a + (p.usage?.outputTokens ?? 0), 0) + judgeOut,
