...
export class FusionStore {
  ...
  private scoreFor(modelId: string, category: string): { score: number; runs: number } {
    const cat = this.db
      .prepare(
        `SELECT AVG(score) AS s, COUNT(*) AS n FROM contributions WHERE model_id = ? AND category = ?`,
      )
  ...
  subjectExpertise(
    modelIds: string[],
    subject: string,
  ): { modelId: string; score: number; runs: number }[] {
  ...
  recentRuns(limit = 20): Array<{
    ...
    return this.db
      .prepare(
        `SELECT id, category, judge_model, created_at FROM runs ORDER BY created_at DESC LIMIT ?`,
      )
      .all(limit) as Array<{
      id: string;
      category: string;
