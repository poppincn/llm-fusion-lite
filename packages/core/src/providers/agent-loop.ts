...
export async function runToolLoop(
  ...
  for (let i = 0; i < MAX_ITERS; i++) {
    const { message, inputTokens: inTok, outputTokens: outTok } = await chatOnce(
      endpoint,
      model,
      messages,
      toolsBody,
    ...
    for (const tc of calls) {
      ...
      messages.push({ role: "tool", tool_call_id: tc.id ?? `call-${toolCalls.length}`, content: out.slice(0, 4000) });
    }
  }
