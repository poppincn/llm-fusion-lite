 ...
 * loops caps concurrent requests. Tune with LLM_FUSION_LITE_OAI_CONCURRENCY (default 4).
 */
const MAX_CONCURRENCY = Math.max(1, Number(process.env.LLM_FUSION_LITE_OAI_CONCURRENCY) || 4);
let active = 0;
...
function release(): void {
    ...
    if (next)
        next(); // hand the permit to the next waiter (active unchanged)
    else active--;
}
...
async function chatOnce(
    ...
    const usage =
        data.usage ? { inputTokens: data.usage.prompt_tokens, outputTokens: data.usage.completion_tokens } : undefined;
