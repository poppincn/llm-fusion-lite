...
/** Container name (override with LLM_FUSION_LITE_SANDBOX). */
function containerName(): string {
    return process.env.LLM_FUSION_LITE_SANDBOX || "llm-fusion-lite-sandbox";
}
...
function stripChrome(out: string): string {
    const noise =
        /^(YOLO mode|Both GOOGLE_API_KEY|Approval mode|Warning:|\[STARTUP\]|Loaded cached|Data collection|Using GOOGLE_API_KEY|warning:|Reading additional input|OpenAI Codex|tokens used|\d+ tokens|exec\b)/i;
    return out
        .split(/\r?\n/)
