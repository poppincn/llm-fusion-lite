...
export function parseConfidence(text: string): number | undefined {
  const m = text.match(/confidence\s*[:=]\s*(\d{1,3})\s*%/i) || text.match(/confidence\s*[:=]\s*(0?\.\d+|1(?:\.0+)?)\b/i);
...
export async function refinePanel(
  ...
  const peerBlock = panel
    .map((p, i) => `${peerLabel(i)}:\n${p.error ? "(no answer)" : p.text}`)
    .join("\n\n");
  ...
  return Promise.all(
    panel.map(async (p, idx) => {
      ...
      return {
        ...
        refined: true,
        usage: res.usage,
        latencyMs: p.latencyMs + res.latencyMs,
      };
    }),
  );
}
  ...
  await Promise.all(
    pairs.map(async ([a, b]) => {
      const res = await provider.complete(judge.model, {
        ...
        signal,
        messages: [
          { role: "system", content: `Compare two candidate answers. Reply ONLY JSON {"winner":"A"|"B"}. Pick the more correct, complete, better-supported one.` },
          { role: "user", content: `QUESTION:\n${promptText(prompt)}\n\nANSWER A:\n${a.text}\n\nANSWER B:\n${b.text}` },
        ],
      });
      let winner = a;
      try {
        ...
        if (String(j.winner ?? "A").toUpperCase().startsWith("B")) winner = b;
      } catch {
...
export async function selectConsistent(
  ...
  const res = await getProvider(judge.provider).complete(judge.model, {
    ...
    signal,
    messages: [
      { role: "system", content: `Several candidate answers to one question are given. Pick the single best, most-consistent-with-the-majority, most correct one. Reply ONLY JSON {"choice": <1-based index>}.` },
      { role: "user", content: `QUESTION:\n${promptText(prompt)}\n\n${list}` },
    ],
  });
  try {
