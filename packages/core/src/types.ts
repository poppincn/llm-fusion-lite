/** Shared message + provider + fusion types for the LLM Fusion Lite engine. */
...
export interface ModelSpec {
    ...
    /**
     * Provider instance this model runs on (ProviderDef.id). When set, the
     * instance's endpoint/credential config is merged onto this spec at load.
     * Any number of instances per adapter is allowed.
     */
    providerId?: string;
...
/**
 * A user-defined provider instance: one backend adapter with its own name and
 * optional endpoint/credential config. Models attach via ModelSpec.providerId.
 * Any number of instances per adapter is allowed (e.g. two OpenAI providers,
 * or several custom ChatCompletion endpoints).
 */
export interface ProviderDef {
    /** Unique instance id (slug). */
    id: string;
    /** Human-friendly name, user-customizable (e.g. "OpenAI Pro"). */
    name: string;
    /** Backend adapter this instance runs on. */
    adapter: ProviderName;
    /** Env var holding this instance's API key (defaults per adapter). */
    apiKeyEnv?: string;
    /** For "openai-compatible" only: chat completions base URL. */
    baseURL?: string;
    /** For "openai-compatible" only: auth header name (default Authorization). */
    apiKeyHeader?: string;
    /** For "openai-compatible" only: extra static HTTP headers. */
    headers?: Record<string, string>;
    /** For "openai-compatible" only: extra request body parameters. */
    extraParams?: Record<string, unknown>;
}
...
export type FusionEvent =
    ...
    | { type: "sme_priors"; subject: string; priors: { modelId: string; score: number; runs: number }[] }
