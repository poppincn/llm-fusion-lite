...
// packages (excluding internal @llm-fusion-lite/* deps, which are bundled in).
...
const pkgs = ["packages/core", "packages/server", "packages/cli"].map(p => read(join(p, "package.json")));

const dependencies = {};
for (const pkg of pkgs) {
    for (const [name, range] of Object.entries(pkg.dependencies ?? {})) {
        if (name.startsWith("@llm-fusion-lite/")) continue; // bundled
        dependencies[name] = range;
    }
}

const release = {
    name: "llm-fusion-lite",
    version: root.version ?? "0.1.0",
    description:
        "LLM Fusion Lite — lightweight multi-model synthesis with adaptive, learned per-subject model strengths. CLI + OpenAI-compatible server + web UI + /fuse skill. A fork of llm-fusion (Era Fusion).",
    type: "module",
    bin: {
        "fusion-lite": "dist/fusion-lite.js",
    ...
    repository: { type: "git", url: "git+https://github.com/jamcaaxian/llm-fusion.git" },
    keywords: [
        "llm",
        "fusion",
        "multi-model",
