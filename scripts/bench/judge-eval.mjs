...
function readJsonl(path) {
  return readFileSync(path, "utf8").split(/\r?\n/).map((l) => l.trim()).filter(Boolean)
    .map((l, i) => { const o = JSON.parse(l); if (!o.id) o.id = `item-${i + 1}`; return o; });
}
...
function majorityCorrect(panelLetters, gold) {
  ...
  for (const k in counts) { if (counts[k] > n) { n = counts[k]; best = k; tie = false; } else if (counts[k] === n) tie = true; }
...
async function snapshot(args) {
  ...
  console.error(`SNAPSHOT — ${items.length} items × ${kPanel} panel sample(s) × ${panelSpecs.length} models @ depth=${depth}`);
  ...
  for (const item of items) {
    ...
    for (let s = 0; s < kPanel; s++) {
      const responses = await Promise.all(panelSpecs.map(async (spec) => {
        const res = await complete(spec, gradedPrompt(item), depth);
        const letter = res.error ? "" : extractLetter(res.text);
        return {
          modelId: spec.id, label: spec.label, text: res.text || "", error: res.error ?? null,
          letter, correct: res.error ? 0 : gradeLetter(res.text, item.answer),
          usage: res.usage ?? null, latencyMs: res.latencyMs,
        };
      }));
      snapshots.push({ panel: responses });
      const mark = responses.map((r) => `${r.modelId.split("-")[0]}:${r.error ? "ERR" : r.letter}${r.correct ? "✓" : ""}`).join(" ");
      console.error(`  ${item.id} s${s + 1}  ${mark}  (gold ${item.answer})`);
    }
    out.items.push({ id: item.id, prompt: item.prompt, answer: item.answer, choices: item.choices ?? null, snapshots });
  }
...
function bootstrapCI(perItem, B = 2000) {
  ...
  const means = [];
  for (let b = 0; b < B; b++) {
    let s = 0; let seed = (b * 2654435761) >>> 0;
    for (let i = 0; i < n; i++) { seed = (seed * 1103515245 + 12345) >>> 0; s += perItem[seed % n]; }
    means.push(s / n);
  }
  means.sort((a, b) => a - b);
...
async function judges(args) {
  ...
  console.error(`JUDGES — ${judgeSpecs.length} judges × ${cache.items.length} items × ${kJudge} sample(s) over cached panels`);
  ...
  for (const it of cache.items) {
    ...
    for (const judge of judgeSpecs) {
      for (const snap of it.snapshots) {
        const panel = snap.panel.map((p) => ({
          modelId: p.modelId, label: p.label, text: p.text, error: p.error,
          provider: getModel(config, p.modelId)?.provider, model: getModel(config, p.modelId)?.model,
          latencyMs: p.latencyMs ?? 0, usage: p.usage ?? undefined,
        }));
        for (let k = 0; k < kJudge; k++) {
          tasks.push((async () => {
            try {
              const analysis = await runJudgeAnalysis(judge, prompt, panel, {});
              const synth = await runJudgeSynthesis(judge, prompt, panel, analysis, { maxTokens: 4096 });
              return { judgeId: judge.id, correct: synth.error ? 0 : gradeLetter(synth.text, it.answer), error: synth.error ?? null };
            } catch (e) {
              return { judgeId: judge.id, correct: 0, error: e instanceof Error ? e.message : String(e) };
            }
          })());
        }
      }
    ...
    detail.push(row);
    console.error(`  ${it.id}  ` + judgeSpecs.map((j) => `${j.id.split("-")[0]}:${(row.judges[j.id].mean * 100).toFixed(0)}%`).join(" "));
  }
  ...
  for (const r of ranked)
    console.log(r.id.padEnd(22) + `${(r.mean * 100).toFixed(1)}%`.padEnd(8) + `[${(r.lo * 100).toFixed(1)}, ${(r.hi * 100).toFixed(1)}]`);
  ...
  if (ranked.length > 1) {
    ...
    for (const r of ranked.slice(1)) {
      ...
      console.log(`  ${top.id} − ${r.id}: +${(ci.mean * 100).toFixed(1)} pts  CI [${(ci.lo * 100).toFixed(1)}, ${(ci.hi * 100).toFixed(1)}]${sig}`);
    }
  }
  ...
  writeFileSync(outPath, JSON.stringify({ cache: cachePath, judges: judgeIds, kJudge, perItem, detail, ranked }, null, 2));
  console.log(`\nWrote ${outPath}`);
}
