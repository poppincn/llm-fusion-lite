# LLM Fusion Lite as an MCP tool

Give any MCP-capable agentic coding stack a **`fuse`** tool — multi-model consensus on demand. The agent calls it when _it_
...
The server runs the engine **in-process** with your local `~/.llm-fusion-lite` config and keys, so it learns per-subject model
...
1. Provider keys in `~/.llm-fusion-lite/.env` (or a configured subscription) — run `fusion-lite doctor` to check.
2. For `agentic: true` only: the sandbox container running — `sandbox/run.sh up` (see [AGENTIC_FUSION.md](AGENTIC_FUSION.md)).

## Install

The server is the `fusion-lite-mcp` bin on the published package, so no separate install — your MCP client launches it via `npx`.

### Claude Code

```bash
claude mcp add llm-fusion-lite -- npx -y llm-fusion-lite fusion-lite-mcp
```

or in `.mcp.json` / `~/.claude.json`:

```json
{ "mcpServers": { "llm-fusion-lite": { "command": "npx", "args": ["-y", "llm-fusion-lite", "fusion-lite-mcp"] } } }
```

### Codex (`~/.codex/config.toml`)

```toml
[mcp_servers.llm-fusion-lite]
command = "npx"
args = ["-y", "llm-fusion-lite", "fusion-lite-mcp"]
```
