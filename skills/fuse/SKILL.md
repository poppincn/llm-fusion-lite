...
- Fusion never runs a model it has no credentials for. If a panelist is skipped (missing key or unauthenticated CLI), that's the credential preflight working as intended — tell the user how to fix it (`fuse doctor`, or the `⚠ skipped` reason), and the run still completes on the credentialed models.
